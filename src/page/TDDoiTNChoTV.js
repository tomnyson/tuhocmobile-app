...
class TDDoiTNChoTV extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    ...
    this.thanhVien = this.props.navigation.getParam('thanhVien', null);
  }
  ...
  async componentDidMount() {
    ...
    var result = await api.getTruongNhomsKhacByThanhVienID(this.app.loginInfo.id, this.thanhVien.id);
    ...
    if (result.code == 200) {
      ...
      if (data.success) {
        ...
        if (data.truongNhomCu != null) {
            this.setState({truongNhomCu: data.truongNhomCu});
        }
  ...
  async onFilterClick() {
    ...
    var result = await api.truongDoanFilterTruongNhom({
      ...
      NotIncludeThisIDTruongNhom: this.state.truongNhomCu.id
    });
  ...
  onChonPress(item) {
    Alert.alert(
        'Đổi Trưởng Nhóm',
        'Đổi qua trưởng nhóm ' + item.hoTen + funcs.getPhapDanh(item),
        [
          {
            text: 'Cancel',
            style: 'cancel',
  ...
  async doiTruongNhom(item) {
    ...
    var result = await api.truongDoanDoiTruongNhom({
      IDThanhVien: this.thanhVien.id,
    ...
    if (result.code == 200) {
      ...
      if (data.success) {
        funcs.back({
          msg: "Đổi trưởng nhóm thành công",
          truongNhomCurrent: this.state.truongNhomCu
        });
      } else {
        funcs.showMsg(data.message);
        this.setState({spinner: false});
    ...
    return(
        <Container>
            ...
            <View style={[s.header.container]}>
                <Button transparent onPress={()=>funcs.back({truongNhomCurrent: this.state.truongNhomCu})}>
                    <IconFontAwesome name="arrow-left" style={styles.iconHeaderLeft}/>
                </Button>
                <Text style={styles.headerText}>Đổi Trưởng Nhóm</Text>
                <View>
            ...
            <View style={[styles.columnCenter, {padding: 10, borderWidth: 1, borderColor: Colors.navbarBackgroundColor}]}>
                <View style={[styles.alignCenter]}>
                    <Text style={{color: Colors.navbarBackgroundColor}}>Thành viên: {this.thanhVien.hoTen}{funcs.getPhapDanh(this.thanhVien)}</Text>
                </View>
                <View style={[styles.alignCenter]}>
                    <Text style={{color: Colors.navbarBackgroundColor}}>Tr.nhóm cũ: {this.state.truongNhomCu.hoTen}{funcs.getPhapDanh(this.state.truongNhomCu)}</Text>
                </View>
            </View>

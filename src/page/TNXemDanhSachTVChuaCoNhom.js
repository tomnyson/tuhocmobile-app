...
class TNXemDanhSachTVChuaCoNhom extends Component {
  constructor(props) {
    super(props);
  ...
  async refreshData() {
    var result = await api.getDSThVChuaCoNhom(this.app.loginInfo.id);
    if (result.code == 200) {
      var data = result.data;
      if (data.success) {
        this.buildListThanhVien(data.listTruongDoanThemVao, data.listThanhVienDangKy, data.listThanhVienOffline, data.listThanhVienBiTuChoi);
      } else {
  ...
  buildListThanhVien(listTruongDoanThemVao, listThanhVienDangKy, listThanhVienOffline, listThanhVienBiTuChoi) {
    this.listTruongDoanThemVao = listTruongDoanThemVao;
    this.listThanhVienDangKy = listThanhVienDangKy;
    this.listThanhVienOffline = listThanhVienOffline;
    this.listThanhVienBiTuChoi = listThanhVienBiTuChoi;
    this.renderThanhVienContent(listTruongDoanThemVao, listThanhVienDangKy, listThanhVienOffline, listThanhVienBiTuChoi);
  }

  renderThanhVienContent(listTruongDoanThemVao, listThanhVienDangKy, listThanhVienOffline, listThanhVienBiTuChoi) {
    var arr = [];
    ...
    if ((typeof listTruongDoanThemVao == "undefined" || listTruongDoanThemVao == null || listTruongDoanThemVao.length == 0)
      ...
      && (typeof listThanhVienOffline == "undefined" || listThanhVienOffline == null || listThanhVienOffline.length == 0)
      && (typeof listThanhVienBiTuChoi == "undefined" || listThanhVienBiTuChoi == null || listThanhVienBiTuChoi.length == 0)){
      arr.push((
      ...
      if (typeof listThanhVienOffline != "undefined" && listThanhVienOffline != null) {
        chan = true;
        for(var i = 0; i < listThanhVienOffline.length; ++i) {
          var item = listThanhVienOffline[i];
          arr.push(this.getJSXLandscapeTableRow(i, item, chan, {them: true, dongY: false, chapNhan: false, tuChoi: false}));
          chan = !chan;
        }
      }

      if (typeof listThanhVienBiTuChoi != "undefined" && listThanhVienBiTuChoi != null) {
        chan = true;
        for(var i = 0; i < listThanhVienBiTuChoi.length; ++i) {
          var item = listThanhVienBiTuChoi[i];
          arr.push(this.getJSXLandscapeTableRow(i, item, chan, {them: false, dongY: false, chapNhan: true, tuChoi: false}));
          chan = !chan;
        }
      }
    }
                  ...
                  <View style={[s.table.align, s.table.width25, s.table.borderRight]}>
                    {
                      actions.them ?
                      (
                        <TouchableOpacity onPress={()=>this.onThemPress(item)}>
                          <Text style={[s.table.text, s.table.textCenter, styles.actionText]}>Thêm</Text>
                        </TouchableOpacity>
                      )
                      : null
                    }
  ...
  async onThemPress(item) {
    this.setState({spinner: true});
    var result = await api.themThanhVienOffline({
        IDThanhVien: item.id,
        IDTruongNhomNew: this.app.loginInfo.id
    });
  ...
  getThanhVien(id) {
    ...
    if (typeof listThanhVienOffline != "undefined" && listThanhVienOffline != null) {
        for(var i = 0; i < this.listThanhVienOffline.length; ++i) {
          var item = this.listThanhVienOffline[i];
          if (item.id == id) {
            return item;
          }
        }
    ...
    if (typeof listThanhVienBiTuChoi != "undefined" && listThanhVienBiTuChoi != null) {
        for(var i = 0; i < this.listThanhVienBiTuChoi.length; ++i) {
          var item = this.listThanhVienBiTuChoi[i];
          if (item.id == id) {
            return item;
          }
        }
    ...
    return(
        <Container>
          ...
          <View style={[s.header.container]}>
            ...
            <Text style={styles.headerText}>Danh sách thành viên chưa có nhóm</Text>
            <View>
          ...
          <Segment style={{backgroundColor: Colors.buttonColor, borderWidth: 1, borderColor: Colors.navbarBackgroundColor}}>
            <Button style={[s.segmentButton]}
              first active={false} onPress={() => funcs.goToWithoutPushStack("TNXemDanhSachThanhVien")}>
              <Text style={[]}>DS T.Viên Đã Vào Nhóm</Text>
            </Button>
            <Button style={[s.segmentButton, s.segmentButtonActive]}
              last active={true} >
              <Text style={[s.segmentTextActive]}>DS T.Viên Chưa Có Nhóm</Text>
            </Button>
          </Segment>
...
export default TNXemDanhSachTVChuaCoNhom;